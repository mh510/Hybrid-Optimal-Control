...
function [traj, obj, theta, time, exitflag, output] = ...
    probabilistic_multiple_shoot(t, mxI, SxI, xG, propfcn, bounds, ...
...
ceq = SX.zeros((nx + (nx^2+nx)/2)*nt, 1);
for i = 1 : nt-1
    % - state mean, chol and input
    mx = mxi(:, i);
    Lxv = Lxvi(:, i);
    Lx = vec2chol(Lxv);
    Sx = Lx*Lx';
    ...
    % - constraints
    ceq((i-1)*(nx + (nx^2 + nx)/2)+1 : i*(nx + (nx^2 + nx)/2)) = ...
        [(mxi(:, i+1) - my)', (Lxvi(:, i+1) - Lyv)']';
end
ceq((nt-1)*(nx + (nx^2 + nx)/2)+1 : nt*(nx + (nx^2 + nx)/2)) = ...
        [(mxi(:, 1) - mxI)', (Lxvi(:, 1) - LxIv)']';
...
if isempty(theta0)
    ...
    Inx = eye(nx);
    Inxv = chol2vec(Inx);
    Lxvi0 = repmat(Inxv, 1, nt);
    theta0 = [mxi0(:)', mui0(:)', Lxvi0(:)']';
end
...
Lxlb = repmat(-inf*ones(nx*(nx+1)/2, 1), 1, nt);
Lxub = repmat(inf*ones(nx*(nx+1)/2, 1), 1, nt);
thetalb = [mxlb(:)', mulb(:)', Lxlb(:)']';
thetaub = [mxub(:)', muub(:)', Lxub(:)']';
...
Lxvi = reshape(theta(nx*nt+nu*nt+1:nx*nt+nu*nt+nx*(nx+1)/2*nt), ...
    nx*(nx+1)/2, nt);
sxi = zeros(nx, nt);
Sxj = zeros(nx, nx, nt);
for i = 1 : nt
